...
export default function DifferenceSection() {
  ...
  return (
    <section style={{ background: C.cream2, padding: isMobile ? "60px 20px" : "120px 56px", borderTop: border.base }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 40, flexWrap: "wrap", marginBottom: isMobile ? 32 : 56 }}>
          <div>
          ...
          {!isMobile && <Sticker color="coral" rotate={-3} size="lg">REAL UNLIMITED, OR NOTHING</Sticker>}
        </div>

        <p style={{ fontSize: isMobile ? 17 : 21, lineHeight: 1.5, maxWidth: 900, margin: "0 0 " + (isMobile ? "32px" : "56px") }}>
        ...
        <div className="grid-3-col">
          {cards.map((card) => (
            <div key={card.badge} style={{ background: "#fff", border: border.thick, borderRadius: 28, padding: isMobile ? 24 : 32, position: "relative", boxShadow: shadow.md, display: "flex", flexDirection: "column", gap: 18, minHeight: isMobile ? "auto" : 360 }}>
              ...
              <h3 style={{ fontFamily: F.display, fontWeight: 800, fontSize: isMobile ? 26 : 34, letterSpacing: "-0.03em", lineHeight: 0.95, margin: 0 }}>{card.title}</h3>
        ...
        {/* VS Table */}
        <div className="vs-table-wrap" style={{ marginTop: isMobile ? 32 : 56 }}>
          <div style={{ background: C.ink, color: C.cream, borderRadius: 32, border: border.thick, boxShadow: shadow.lg, overflow: "hidden", minWidth: isMobile ? 560 : "auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 24, marginTop: isMobile ? 32 : 56 }}>
